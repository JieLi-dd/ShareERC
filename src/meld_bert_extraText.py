    ...
    def preprocess_data(self):
        ...
        for dia_id in list(int2name.keys()):   #demo [:4]
            ...
            for utt_id in int2name[dia_id]:
                ...
                labels.append(load_dict[utt_id]['label'])
            ...
            InputFeatures(input_ids=input_ids,
                          ...
                          sep_mask=sep_mask))
        features_labels = {'features':features, 'labels':labels}
        return features_labels

class IEMOCAP():
    def __init__(self, load_anno_csv,pretrainedBertPath, iemocap_text_path, set_name):
        # data path
        self.load_anno_csv = load_anno_csv  #
        self.pretrainedBertPath = pretrainedBertPath
        self.iemocap_text_path = iemocap_text_path
        self.set_name = set_name
    ...
    def preprocess_data(self):
        ...
        csv_path = osp.join(self.load_anno_csv, 'IEMOCAP_' + self.set_name + '.csv')
        int2name = make_text_dia(csv_path)  #

        text_root = osp.join(self.iemocap_text_path, self.set_name + '_text.json')
        with open(text_root, 'r') as load_f:
            load_dict = json.load(load_f)
        ...
        for dia_id in list(int2name.keys()):  # demo [:4]
            ...
            if self.pretrainedBertPath.split('/')[-1] == 'roberta-large':
                tokens_temp = _truncate_seq_pair(temp_utt,
                                                 Max_seq_length - 19 * 2)  # 这是计算单独的时候, 之后每个dialogue下的utterance concat一起的时候, 每个utterance会加上</s>A</s>
            elif self.pretrainedBertPath.split('/')[-1] == 'bert-large':
                tokens_temp = _truncate_seq_pair(temp_utt,
                                                 Max_seq_length - 19)  # 这是计算单独的时候, 之后每个dialogue下的utterance concat一起的时候, 每个utterance后面要加上[sep]
